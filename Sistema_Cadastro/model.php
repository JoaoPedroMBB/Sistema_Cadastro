...
class Model{
    ...
    private $username = "id20263066_root";
    private $pass = "dMPJ/7%>Q5CEG&BX";
    private $database = "id20263066_aula_sistema_cadastro";
    private $conn;
        ...
        if(isset($_POST['submit'])){
            ...
            if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['whats']) && isset($_POST['address'])){
                if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['whats']) && !empty($_POST['address'])){
                     ...
                     $query = "INSERT INTO cadastros (name,email,whatssap,endereco) VALUES ('$name','$email','$whats','$address')";
    ...
    public function update($data){
        //var_dump($data);
        $query = "UPDATE cadastros SET name='$data[name]',email='$data[email]',
        whatssap = '$data[whatssap]',endereco = '$data[endereco]' WHERE id = '$data[id]'";
        if ($sql = $this->conn->query($query)){
            return true;
        }else{
            return false;
